...
def sign_up(request):
    try:
        AppUser.objects.create_user(first_name=request.data['name'], last_name=request.data['lastName'], job_title=request.data['jobTitle'],
...
def jobs_applied_for(request):
    ...
    if request.method == 'POST':
        try:
            ...
            new_job=AppliedJobs.objects.create(user= user, company_link= company_link, description= job_description, company_name= company_name, job_title=job_title)
            new_job.save()
...
def interviews(request):
    ...
    if request.method == "POST":
        try:
            user= request.user
            company_name= request.data['company_name']
            interview=Interview.objects.create(user=user, company_name= company_name)
            interview.save() 
...
def forums(request):
    ...
    if request.method == "POST":
        try:
            ...
            forum=Forum.objects.create(title=title, user=user, company_name= company_name, job_title = job_title, description = description)
            forum.save()
