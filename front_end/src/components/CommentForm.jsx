...
export default function CommentForm({title, setTitle, handleTitleEntry, text, setText, handleSubmit, submitLabel}) {
  ...
  return (
      <div class="bg-surface-50 text-basicSurface-500 shadow flex flex-col justify-between sm:rounded-lg block-post-composer">
        <div class="flex-1 px-4 py-5 sm:p-6 flex space-x-4 items-center">
          ...
          <button onClick={toggleCommentModal} type="button" class="bg-surface-100 hover:bg-surface-200 text-basicSurface-500 rounded-md p-2 px-3 h-10 flex-grow text-left">What's on your mind?</button>
          <Modal
            show={lgShow}
            onHide={() => setLgShow(false)}
          >
            <div className="p-4">
              <div class="flex flex-row justify-content-between"><h4 >Create post</h4><div class="d-flex space-x-4 align-self-center"> <BsGear /> <FaCompress /> <GrClose /></div>  </div> 
            </div>
